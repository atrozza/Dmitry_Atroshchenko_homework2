...
def common_price(m, n, s, l):
    if m == str(m) or n == str(n) or s == str(s) or l == str(l):
        return False
    else:
        m, n = float(m), float(n)
        s, l = int(s), int(l)
...
def longest_word(sentence):
    if not sentence or str(sentence).isdigit():
        return False
    else:
        punctuation = [',', '.', '!', '?']
...
def uniques(repeating_string):
    if not repeating_string or str(repeating_string).isdigit():
        return False
    else:
...
def count_string_capitalization(mixed_string):
    if mixed_string == True or isinstance(mixed_string, list) or str(mixed_string).isdigit():
        return False
    else:
